...
public class WatchingThredPool extends FixedThreadPool implements Runnable {

    private static final Logger LOGGER = LoggerFactory.getLogger(WatchingThredPool.class);

    // 定义线程池的阀值
    private static final double ALARM_PERCENT = 0.90;
    private final Map<URL, ThreadPoolExecutor> THREAD_POOLS = new ConcurrentHashMap<>();

    public WatchingThredPool() {
        // 每隔三秒打印线程使用情况
    ...
    public void run() {
        ...
        for (Map.Entry<URL, ThreadPoolExecutor> entry : THREAD_POOLS.entrySet()) {
            ...
            final int poolSize = threadPoolExecutor.getCorePoolSize();
            final double userPercent = activeCount / (poolSize * 1.0);
